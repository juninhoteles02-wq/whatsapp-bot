...
from ms_graph import criar_solicitacao_planilha
...
# Lista de números autorizados a usar o bot (formato E.164 sem "+", ex.: 5521999999999).
# Separe por vírgula na variável de ambiente. Se ficar vazia, o bot aceita qualquer número
# (comportamento antigo) - defina essa variável para travar o acesso.
NUMEROS_AUTORIZADOS = [
    n.strip() for n in os.getenv("NUMEROS_AUTORIZADOS", "").split(",") if n.strip()
]

# Nome que aparece como "solicitante GMS" nas solicitações criadas pelo bot.
SOLICITANTE_PADRAO = os.getenv("SOLICITANTE_PADRAO", "ROGÉRIO JÚNIOR")
...
def detectar_intencao(texto):
    ...
    # solicitação (integração com a planilha de follow-up)
    if texto_n.startswith("solicitar "):
    ...
    # agenda - criação
    # IMPORTANTE: este bloco precisa ser checado ANTES do "texto_n.startswith('agenda')"
    # abaixo, porque "agendar"/"agende" também começam com "agenda" e antes caíam sempre
    # em consultar_agenda, impedindo a criação de eventos.
    sinais_criar_evento = [
        "agendar",
        "agende",
        "marcar",
...
# =========================
# SOLICITAÇÃO (planilha de follow-up) - interpretação
# =========================
def interpretar_criar_solicitacao(texto):
    """
    Formato esperado:
        Solicitar <responsável>, contrato <contrato>: <pedido>
        Solicitar <responsável>: <pedido>   (contrato opcional)

    Ex.: "Solicitar Gabrielly, contrato Atlantic Golf: comprar capacete novo"
         "Solicitar Gabrielly: comprar capacete novo"
    """
    m = re.match(
        r'^\s*solicitar\s+([^,:]+?)\s*(?:,\s*contrato\s+([^:]+?))?\s*:\s*(.+)$',
        texto.strip(),
        flags=re.IGNORECASE,
    )
    if not m:
    ...
    responsavel = m.group(1).strip()
    contrato = (m.group(2) or "").strip()
    pedido = m.group(3).strip()

    if not responsavel or not pedido:
        return None

    return {"responsavel": responsavel, "contrato": contrato, "pedido": pedido}
...
def receber_mensagem():
    try:
        ...
        # Trava de segurança: só processa mensagens de números autorizados.
        # Se NUMEROS_AUTORIZADOS estiver vazio (variável de ambiente não configurada),
        # o bot mantém o comportamento antigo e aceita qualquer número.
        if NUMEROS_AUTORIZADOS and numero not in NUMEROS_AUTORIZADOS:
            print(f"Número não autorizado tentou usar o bot: {numero}")
            return "ok", 200
        ...
        # 6) criar solicitação (grava na planilha de follow-up)
        if intencao == "criar_solicitacao":
            dados = interpretar_criar_solicitacao(texto)
            if not dados:
                resposta = (
                    "Não entendi o pedido. Use o formato:\n"
                    "Solicitar <responsável>, contrato <contrato>: <pedido>\n"
                    "Ex.: Solicitar Gabrielly, contrato Atlantic Golf: comprar capacete novo"
                )
            else:
                try:
                    item = criar_solicitacao_planilha(
                    ...
                    resposta = (
                        f"Solicitação registrada (ITEM {item}):\n"
                        f"Responsável: {dados['responsavel']}\n"
                        + (f"Contrato: {dados['contrato']}\n" if dados["contrato"] else "")
                        + f"Pedido: {dados['pedido']}"
                    )
                except Exception as e:
                    resposta = f"Não consegui registrar na planilha agora: {str(e)}"
